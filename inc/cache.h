...
//	mshr_full = 0;
    ...
    void add_nonfifo_queue(PACKET_QUEUE *queue, PACKET *packet), //@Vishal: Updated from add_mshr
		...
		stlb_replacement_final_stats(),
         ...
         l1d_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint8_t critical_ip_flag),//, int prefetcher_type), //#YVR Added a prefetcher type field //, uint64_t prefetch_id),
