...
//#define final_pref 1                     // to choose the best prefetcher
...
//int final_pref = 1;  //#YVR to use after phase length
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // read hit
                ...
                //int prefetcher_selecting = 1;   //#YVR Current Prefetcher
                ...
                if (RQ.entry[index].type == LOAD) {
                    ...
                    if (cache_type == IS_L1D){
                        // //Change if the phase is during warmup
                        // while(warmup_complete[read_cpu] && prefetcher_selecting!=4){   //#YVR can also put warmup_complete[0] cause single core 

                        //     int z=0;                        //#YVR for for loop
                        //     float prev_accuracy = 0;
                        //     float cur_accuracy;
                        //     if(z<PHASE_LENGTH){
                        //         l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 1, RQ.entry[index].type, RQ.entry[index].critical_ip_flag, prefetcher_selecting); //#YVR for IP_STRIDE send 1	// RQ.entry[index].instr_id);
                        //         z++;
                        //         }
                        //     else{
                        //         cur_accuracy = (pf_useful*100/pf_lower_level); //#YVR Maybe itll calculate right prefetch accuracy
                        //         pf_useful = 0;     //#YVR can i reset these?
                        //         pf_lower_level = 0;//resetting these for a inpendent accuracy each phase
                        //         if  (cur_accuracy > prev_accuracy) {
                        //             prev_accuracy = cur_accuracy;
                        //             final_pref = prefetcher_selecting;
                        //             prefetcher_selecting++;
                        //         }else{
                        //             prev_accuracy = cur_accuracy;
                        //             prefetcher_selecting++;
                        ...
                        //     }
                        // }
                        l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 1, RQ.entry[index].type, RQ.entry[index].critical_ip_flag); // final_pref); //#YVR for IP_STRIDE send 1	// RQ.entry[index].instr_id);
                    ...
                    if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss
                        ...
                        if (miss_handled) {
                            ...
                            if (RQ.entry[index].type == LOAD) {
                                ...
                                if (cache_type == IS_L1D) 
                                    l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, RQ.entry[index].critical_ip_flag); //,final_pref);	//RQ.entry[index].instr_id); //#YVR added final_perf - selected prefetcher
                    ...
                    if (way >= 0) { // prefetch hit
                        ...
                        if(PQ.entry[index].pf_origin_level < fill_level)
                        {
                            if (cache_type == IS_L1D)
                            {
                                ...
                                assert(0);
                                l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 1, PREFETCH, PQ.entry[index].critical_ip_flag);//,final_pref);	//, PQ.entry[index].prefetch_id); //#YVR added final_perf - selected prefetcher
                            }
                            else if ((cache_type == IS_L2C) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].instruction == 0) && (RQ.entry[index].type != LOAD_TRANSLATION) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].type != TRANSLATION_FROM_L1D))
                            {
                            ...
                            if (lower_level) {
                                if (cache_type == IS_LLC) {
                                    ...
                                    if (lower_level->get_occupancy(3, PQ.entry[index].address) == lower_level->get_size(3, PQ.entry[index].address))
                                        ...
                                        {
                                            if (cache_type == IS_L1D)
                                                l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 0, PREFETCH, PQ.entry[index].critical_ip_flag);//, final_pref);	// PQ.entry[index].prefetch_id); //#YVR added final_perf - selected prefetcher
                                            else if ((cache_type == IS_L2C) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].instruction == 0) && (RQ.entry[index].type != LOAD_TRANSLATION) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].type != TRANSLATION_FROM_L1D))
                                            {
